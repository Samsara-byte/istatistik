...
export default function GenelDestekTable() {
    ...
    async (sk = sort.key, sd = sort.dir) => {
      ...
      try {
        ...
        const [r1, r2] = await Promise.all([
          fetch(`${BASE}/api/genel-destek?${p}`).then((r) => r.json()),
          fetch(`${BASE}/api/genel-destek/ozet`).then((r) => r.json()),
        ]);
        setData(r1.data || []);
        setOzet(r2.data || []);
      } finally {
  ...
  const exportExcel = useCallback(async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Genel Destekler");
      XLSX.writeFile(
        wb,
        `genel_destek_destek${yilFil ? "_" + yilFil : ""}.xlsx`,
      );
    } finally {
      setExporting(false);
    }
