...
export default function MapPanel({
  ...
  return (
    <div className="mp">
      {/* Header — tıklanınca collapse */}
      <div className="mp-head" onClick={() => setCollapsed(c => !c)}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span className={`mp-toggle-icon${collapsed ? ' collapsed' : ''}`}>▼</span>
        </div>
      </div>

      {/* Collapsible içerik */}
      <div className={`mp-collapsible${collapsed ? ' collapsed' : ''}`}>
        {/* SVG Map */}
            ...
            <g filter={`url(#sf-${pageId})`}>
              {DISTRICT_PATHS.map(dp => (
                <path
                  ...
                  onClick={(e) => { e.stopPropagation(); onPickDistrict(dp.id, dp.name); }}
                />
              ))}
              {DISTRICT_PATHS.map(dp => (
                <text
