...
export default function AlanBazliTable() {
    ...
    async (sk = sort.key, sd = sort.dir) => {
      ...
      try {
        ...
        const [r1, r2] = await Promise.all([
          fetch(`${BASE}/api/alan-bazli?${p}`).then((r) => r.json()),
          fetch(`${BASE}/api/alan-bazli/ozet`).then((r) => r.json()),
        ]);
        setData(r1.data || []);
        setOzet(r2.data || []);
      } finally {
  ...
  const exportExcel = useCallback(async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Alan Bazlı Destekler");
      XLSX.writeFile(wb, `alan_bazli_destek${yilFil ? "_" + yilFil : ""}.xlsx`);
    } finally {
      setExporting(false);
    }
            ...
            <tbody>
              {loading ? (
                ...
                <EmptyRow
                  ...
                  text="Veri yok — İçeri Aktar ile alan bazlı destek verisi yükleyin"
                />
              ) : (
                <>
                  {destekler.map((ad, ri) => (
