...
from app.helpers import build_where, paginate, safe_sort, sort_direction
...
async def list_uretim(
    ...
    offset = (page - 1) * limit
    # alias="u" → JOIN sorgularında tablo prefix'i için
    where_u, params   = build_where(yil=yil, ilce=ilce, koy=koy, urun=urun,
                                    tarim_sekli=tarim_sekli, uretim_cesidi=uretim_cesidi, alias="u")
    where_p, params_p = build_where(yil=yil, ilce=ilce, koy=koy, urun=urun,
...
async def ozet_uretim(
    ...
    tot = (
        await db.execute(
            text(f"SELECT ROUND(SUM(ekili_alan)::numeric,2) AS alan, COUNT(*)::int AS kayit FROM uretim {where}"),
            params,
        )
    ).mappings().fetchone()
...
async def list_urunler(
    ...
    rows = (
        await db.execute(
            text(f"SELECT urun, ROUND(SUM(ekili_alan)::numeric,2) AS toplam_alan FROM uretim {where} GROUP BY urun ORDER BY toplam_alan DESC"),
            params,
        )
    ).mappings().all()
