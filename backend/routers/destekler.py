"""
Özet destek tablolarına ait GET / DELETE endpoint'leri.

Her destek tipi için aynı pattern'i uygular:
  GET  /api/{route}
  GET  /api/{route}/ozet
  DELETE /api/{route}/temizle
"""
from __future__ import annotations

from typing import Optional

from fastapi import APIRouter, Depends, Query
from sqlalchemy import text
from sqlalchemy.ext.asyncio import AsyncSession

from app.database import get_db

router = APIRouter(tags=["Destek Tabloları"])


def _make_destek_routes(
    route: str,
    table: str,
    key_field: str,
    key_label: str,
) -> None:
    """Register list, ozet, and temizle endpoints for a support table."""

    @router.get(f"/api/{route}", name=f"list_{route.replace('-','_')}")
    async def _list(
        yil:   Optional[int] = Query(None),
        limit: int           = Query(9_999, le=99_999),
        db:    AsyncSession  = Depends(get_db),
    ):
        clauses: list[str] = []
        params: dict = {}
        if yil:
            clauses.append("yil = :yil")
            params["yil"] = yil
        where = ("WHERE " + " AND ".join(clauses)) if clauses else ""
        sql = f"""
            SELECT {key_field}, yil, tutar_tl
            FROM {table} {where}
            ORDER BY {key_field} ASC, yil ASC
            LIMIT :limit
        """
        rows = (await db.execute(text(sql), {**params, "limit": limit})).mappings().all()
        return {"data": [dict(r) for r in rows], "total": len(rows)}

    @router.get(f"/api/{route}/ozet", name=f"ozet_{route.replace('-','_')}")
    async def _ozet(db: AsyncSession = Depends(get_db)):
        sql = f"""
            SELECT yil,
                   SUM(tutar_tl)::numeric         AS toplam_tl,
                   COUNT(DISTINCT {key_field})::int AS {key_label}_sayisi
            FROM {table}
            GROUP BY yil ORDER BY yil DESC
        """
        rows = (await db.execute(text(sql))).mappings().all()
        return {"data": [dict(r) for r in rows]}

    @router.delete(f"/api/{route}/temizle", name=f"temizle_{route.replace('-','_')}")
    async def _temizle(
        yil: Optional[int] = Query(None),
        db:  AsyncSession  = Depends(get_db),
    ):
        async with db.begin():
            if yil:
                r = await db.execute(text(f"DELETE FROM {table} WHERE yil = :y"), {"y": yil})
            else:
                r = await db.execute(text(f"DELETE FROM {table}"))
        return {"silinen": r.rowcount}


# Register all four destek table routes
_make_destek_routes("alan-bazli",         "alan_bazli_destek",  "destek_adi", "destek")
_make_destek_routes("fark-prim",          "fark_prim_destek",   "kategori",   "kategori")
_make_destek_routes("hayvancilik-destek", "hayvancilik_destek", "destek_adi", "destek")
_make_destek_routes("genel-destek",       "genel_destek",       "destek_adi", "destek")
