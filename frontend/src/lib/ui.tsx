...
// villages.json doğrudan import edildi — boş map yerine gerçek veri
...
// ── Form helpers ─────────────────────────────────────────────────
export const Label = memo(function Label({ children }: { children: ReactNode }) {
  return (
    <label style={{
...
export function Sel({
  label, value, onChange, children, minWidth = 110,
}: {
  ...
  return (
    <div style={{ flex: 1, minWidth }}>
      <Label>{label}</Label>
      <select value={value} onChange={e => onChange(e.target.value)} style={SEL_BASE}>
        {children}
      </select>
    </div>
  );
...
export function Inp({
  label, value, onChange, placeholder, minWidth = 110,
}: {
  label: string; value: string; onChange: (v: string) => void;
...
// ── Layout wrappers ──────────────────────────────────────────────
export const FilterBar = memo(function FilterBar({ children }: { children: ReactNode }) {
  return (
    <div style={{
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end" }}>
      <button
        ...
        onMouseEnter={e => {
          e.currentTarget.style.borderColor = "var(--gm)";
  ...
  return (
    <button
      ...
      style={{
        ...
        opacity: loading ? 0.6 : 1, transition: "opacity .2s, background .15s",
        whiteSpace: "nowrap",
      }}
    >
      {loading ? "⏳" : "⬇"} Excel
...
const SortIcon = memo(function SortIcon({
  ...
  return (
    <span style={{
      ...
      marginLeft: 4, verticalAlign: "middle", lineHeight: 1,
...
export function SortableTh({
  label, sortKey, currentSort, onSort, align = "right", width,
}: {
  ...
  return (
    <th
      ...
      title={`${label} — sıralamak için tıklayın`}
      style={{
...
export const Pagination = memo(function Pagination({
  page, pages, total, onPage, unit = "kayıt",
}: {
  ...
  const btn = (label: ReactNode, p: number, dis: boolean) => (
    <button
      key={String(label)}
      onClick={() => !dis && onPage(p)}
      disabled={dis}
      style={{
        minWidth: 28, height: 28, border: "1.5px solid",
        borderColor: p === page && typeof p === "number" ? "var(--gd)" : "var(--br2)",
        borderRadius: 5, fontFamily: "inherit", fontSize: 11,
...
export function useSortState(defaultKey = "", defaultDir: SortDir = null) {
  ...
  const resetSort = useCallback(() => {
    setSort({ key: defaultKey, dir: defaultDir });
  }, [defaultKey, defaultDir]);

  return { sort, onSort, resetSort };
}

export function applySortRows<T extends Record<string, unknown>>(
  rows: T[],
...
// ── Loading / Empty ──────────────────────────────────────────────
export const LoadingRow = memo(function LoadingRow({ cols }: { cols: number }) {
  return (
    <tr>
      <td colSpan={cols} style={{ padding: 40, textAlign: "center", color: "var(--mu)", fontSize: 13 }}>
        ⏳ Yükleniyor…
      </td>
    </tr>
  );
...
export const EmptyRow = memo(function EmptyRow({
  cols, text = "Sonuç bulunamadı",
}: { cols: number; text?: string }) {
  return (
    <tr>
      <td colSpan={cols} style={{ padding: 36, textAlign: "center", color: "var(--mu)", fontStyle: "italic", fontSize: 13 }}>
        {text}
      </td>
    </tr>
  );
