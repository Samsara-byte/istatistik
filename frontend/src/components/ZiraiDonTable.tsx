...
import { api, type ZiraiDonRow } from "@/lib/api";
...
type Col = { key: keyof ZiraiDonRow; label: string; isText?: boolean };
...
const COLS: Col[] = [
  ...
  { key: "hasar_orani_yuzde", label: "Hasar Oranı (%)" },
  { key: "birim_maliyet_tl",  label: "Birim Maliyet (₺)" },
  { key: "etkilenen_alan_da", label: "Etkilenen Alan (da)" },
  { key: "toplam_masraf_tl",  label: "Toplam Masraf (₺)" },
];

const COL_COLORS: Record<string, string> = {
  isletme_sayisi:    "#1a5276",
  hasar_orani_yuzde: "#6b2020",
  birim_maliyet_tl:  "#4a5568",
  etkilenen_alan_da: "#2a3d8b",
  toplam_masraf_tl:  "#4a2070",
};

export default function ZiraiDonTable({
  defaultIlce = "",
  defaultKoy  = "",
}: {
  defaultIlce?: string;
  ...
  const [data,     setData]     = useState<ZiraiDonRow[]>([]);
  ...
  const { sort, onSort, resetSort } = useSortState("toplam_masraf_tl", "desc");
    ...
    async (pg: number, sk = sort.key, sd = sort.dir) => {
      ...
      try {
        const [res, oz, top] = await Promise.all([
          ...
          api.ziraiDonOzet({ yil: parseInt(yil), ilce: ilce || undefined, group_by: "ilce" }),
          api.ziraiDonToplam({ yil: parseInt(yil), ilce: ilce || undefined }),
        ]);
        ...
        for (const row of oz.data) {
          ...
          for (const k of ["isletme_toplam", "alan_toplam", "masraf_toplam"]) {
  ...
  const exportExcel = useCallback(async () => {
    setExporting(true);
    try {
      const res = await api.listZiraiDon({
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Zirai Don");
      XLSX.writeFile(wb, `zirai_don_${yil}${ilce ? "_" + ilce : ""}.xlsx`);
    } finally { setExporting(false); }
  }, [yil, ilce, koy, urun]);

  const ozetKartlar = [
    { label: "Toplam Masraf (₺)",     val: fmt(ozet.masraf_toplam),  color: "#4a2070" },
    { label: "Etkilenen Alan (da)",   val: fmt(ozet.alan_toplam),    color: "#2a3d8b" },
  ...
  return (
    <div className="dc" style={{ marginTop: 16 }}>
      ...
      <TableHeader>
        <h2 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: "var(--gd)" }}>Zirai Don Desteği</h2>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          ...
          <tbody>
            {loading ? (
              ...
              <EmptyRow cols={COLS.length + 1} text="Veri yok — İçeri Aktar ile zirai don verisi yükleyin" />
            ) : (
              <>
                {data.map((row, ri) => (
                  <tr key={row.id}
                  ...
                  >
                    ...
                    {COLS.map((col) => {
                      const v = row[col.key as keyof ZiraiDonRow];
