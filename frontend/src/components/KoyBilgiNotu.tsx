...
// ─── Mini UI bileşenleri ──────────────────────────────────────────
const GRN = "#17472f", GRN2 = "#2d6a4f", GOLD = "#a07010", BLUE = "#1a4a6b";
...
export default function KoyBilgiNotu() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const ok = async (url: string): Promise<any> => {
    ...
    try {
      ...
      if (uretim.length > 0) {
        const ws2 = XLSX.utils.aoa_to_sheet([
          ...
          ...uretim.map(r => [r.urun, GRUP_LABEL[getGrup(r.urun)] ?? getGrup(r.urun), r.tarim_sekli, r.ekili_alan]),
        ]);
  ...
  }, [ilce, koy, yil, gt, toplamAlan, hayv, sutT, bdT, abT, fpT, hdT, gdT, genelT, cks, uretim, bugun]);

  return (
    <div style={{ background: "#edf0ec", minHeight: "100vh", paddingBottom: 40 }}>

      {/* Kontrol Paneli */}
      <div className="no-print" style={{ background: "linear-gradient(135deg,#0e2d1e,#1a6b3a)", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 10, padding: "8px 20px 10px", flexWrap: "wrap" }}>
          {/* Yıl */}
          <div>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,.45)", fontWeight: 700, textTransform: "uppercase", letterSpacing: .9, marginBottom: 3 }}>Yıl</div>
          ...
          {/* İlçe */}
          <div>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,.45)", fontWeight: 700, textTransform: "uppercase", letterSpacing: .9, marginBottom: 3 }}>İlçe</div>
          ...
          {/* Köy */}
          <div>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,.45)", fontWeight: 700, textTransform: "uppercase", letterSpacing: .9, marginBottom: 3 }}>Köy</div>
              ...
              <Card title="Hayvansal Üretim" icon="🐄">
                {hayv ? (
                  <>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 10px", background: "#f0f7f3", borderTop: `1.5px solid ${GRN2}44` }}>
      ...
      <style>{`
        @media print {
          ...
          header, nav, aside, footer, .no-print, .topnav, .app > aside { display: none !important; }
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          body { background: white !important; margin: 0; padding: 0; }
          #bilgi-notu-print { max-width: 100% !important; margin: 0 !important; padding: 0 !important; }
          #bilgi-notu-print > div[style*="grid"] { display: grid !important; grid-template-columns: 1fr 1fr 1fr !important; gap: 6px !important; }
          div[style*="pageBreakInside"] { page-break-inside: avoid !important; }
          #bilgi-notu-print { font-size: 9pt !important; }
        }
      `}</style>
    </div>
  );
