...
import { api, type TemelDestekRow } from "@/lib/api";
...
type Col = { key: keyof TemelDestekRow; label: string; isText?: boolean };
...
export default function TemelDestekTable({
  defaultIlce = "",
  defaultKoy  = "",
}: {
  defaultIlce?: string;
  ...
  const [data,      setData]     = useState<TemelDestekRow[]>([]);
    ...
    async (pg: number, sk = sort.key, sd = sort.dir) => {
      ...
      try {
        const [res, oz, top] = await Promise.all([
          ...
          api.temelDestekOzet({ yil: parseInt(yil), ilce: ilce || undefined, group_by: "ilce" }),
          api.temelDestekToplam({ yil: parseInt(yil), ilce: ilce || undefined }),
        ]);
        setData(res.data);
        setTotal(res.total);
        setPages(res.pages);
  ...
  const exportExcel = useCallback(async () => {
    setExporting(true);
    try {
      const res = await api.listTemelDestek({
        yil: parseInt(yil), ilce: ilce || undefined,
        koy: koy || undefined, urun_grubu: urunGrubu || undefined, limit: 50000,
      });
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Temel Destek");
      XLSX.writeFile(wb, `temel_destek_${yil}${ilce ? "_" + ilce : ""}.xlsx`);
  ...
  return (
    <div className="dc" style={{ marginTop: 16 }}>
      ...
      <TableHeader>
        <h2 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: "var(--gd)" }}>Temel Destek</h2>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          ...
          <tbody>
            {loading ? (
              ...
              <EmptyRow cols={COLS.length + 1} text="Veri yok — İçeri Aktar ile temel destek verisi yükleyin" />
            ) : (
              <>
                {data.map((row, ri) => (
                  <tr key={row.id}
                  ...
                  >
                    ...
                    {COLS.map((col) => {
                      const v = row[col.key as keyof TemelDestekRow];
