...
const MapPanel = memo(function MapPanel({
  ...
  const villages = useMemo(() => {
    ...
    const key = Object.keys(VILLAGE_MAP).find(
      k => k.toLocaleUpperCase("tr-TR") === activeDistrictName.toLocaleUpperCase("tr-TR"),
    );
    ...
    <div className="mp">
          ...
          <g filter={`url(#sf-${pageId})`}>
            {DISTRICT_PATHS.map(dp => (
              <path
                ...
                onMouseEnter={e => setTooltip({ x: e.clientX + 14, y: e.clientY - 10, text: dp.name })}
      ...
      <div className="mp-controls">
        ...
        {(activeDistrictName || selectedVillage) && (
          <button onClick={handleClear} className="mp-clear-btn">
            ✕ Temizle
          </button>
        )}
      </div>
      ...
      {activeDistrictName && (
        <div className="mp-info">
          ...
          {selectedVillage && (
            <span className="mp-info-village">📍 {selectedVillage}</span>
          )}
        </div>
      )}
    </div>
