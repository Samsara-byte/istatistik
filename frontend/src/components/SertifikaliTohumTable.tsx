...
import { api, type SertifikaliTohumRow } from "@/lib/api";
...
type Col = { key: keyof SertifikaliTohumRow; label: string; isText?: boolean };
...
const COLS: Col[] = [
  ...
  { key: "destekleme_miktari_tl", label: "Destekleme Miktarı (₺)" },
];

const COL_COLORS: Record<string, string> = {
  isletme_sayisi:        "#1a5276",
  destekleme_alani_da:   "#2a3d8b",
  destekleme_miktari_tl: "#4a2070",
};

export default function SertifikalıTohumTable({
  defaultIlce = "",
  defaultKoy  = "",
}: {
  defaultIlce?: string;
  ...
  const [data,     setData]     = useState<SertifikaliTohumRow[]>([]);
  ...
  const { sort, onSort, resetSort } = useSortState("destekleme_miktari_tl", "desc");
    ...
    async (pg: number, sk = sort.key, sd = sort.dir) => {
      ...
      try {
        const [res, oz, top] = await Promise.all([
          ...
          api.sertifikaliTohumOzet({ yil: parseInt(yil), ilce: ilce || undefined, group_by: "ilce" }),
          api.sertifikaliTohumToplam({ yil: parseInt(yil), ilce: ilce || undefined }),
        ]);
        setData(res.data);
        setTotal(res.total);
        setPages(res.pages);
        ...
        for (const row of oz.data) {
          ...
          for (const k of ["isletme_toplam", "alan_toplam", "destek_toplam"]) {
  ...
  const exportExcel = useCallback(async () => {
    setExporting(true);
    try {
      const res = await api.listSertifikaliTohum({
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Sertifikalı Tohum");
      XLSX.writeFile(wb, `sertifikali_tohum_${yil}${ilce ? "_" + ilce : ""}.xlsx`);
    } finally { setExporting(false); }
  }, [yil, ilce, koy, urun]);

  const ozetKartlar = [
    { label: "Destekleme (₺)",       val: fmt(ozet.destek_toplam),   color: "#4a2070" },
    { label: "Destekleme Alanı (da)", val: fmt(ozet.alan_toplam),     color: "#2a3d8b" },
  ...
  return (
    <div className="dc" style={{ marginTop: 16 }}>
      ...
      <TableHeader>
        <h2 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: "var(--gd)" }}>Sertifikalı Tohum Kullanım Desteği</h2>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          ...
          <tbody>
            {loading ? (
              ...
              <EmptyRow cols={COLS.length + 1} text="Veri yok — İçeri Aktar ile sertifikalı tohum verisi yükleyin" />
            ) : (
              <>
                {data.map((row, ri) => (
                  <tr key={row.id}
                  ...
                  >
                    ...
                    {COLS.map((col) => {
                      const v = row[col.key as keyof SertifikaliTohumRow];
